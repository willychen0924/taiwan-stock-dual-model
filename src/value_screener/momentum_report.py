...
<body><div class="wrap">
<div class="hero"><div><h1>台股營運動能篩選</h1><p>市場日 {html.escape(meta['latest_market_date'])}　·　財報季 {html.escape(meta['latest_financial_quarter'])}　·　研究候選，不是買進建議</p></div><div class="hero-status {html.escape(str(meta.get('model_status') or 'UNKNOWN').lower())}">{html.escape(str(meta.get('model_status') or 'UNKNOWN'))}</div></div>
{checks_panel}
...
<p class="note">營運動能分數只負責縮小研究範圍。3M營收與營收加速度採各公司截至報表日最新可用期別；流動性硬門檻仍以20日均成交額判定，表格僅改顯示20日均量。排名不是預期報酬或買進建議；治理、競爭優勢與成長延續性仍須以年報、法說及公開資訊查證。</p>
