...
def write_momentum_reports(result: dict[str, Any], reports_root: Path) -> dict[str, Path]:
    ...
    html_path.write_text(_build_momentum_html(result), encoding="utf-8")
...
def _build_momentum_html(result: dict[str, Any]) -> str:
...
td.summary{{min-width:30rem;white-space:normal;line-height:1.6}} tr:hover td{{background:#fffbeb}} .copy{{line-height:1.8;color:#40564f}} .copy h3{{color:#b45309;margin-bottom:6px}} .copy ol{{padding-left:22px}}
...
<body><div class="wrap">
<div class="hero"><h1>台股高品質營運動能篩選</h1><p>市場資料 {html.escape(meta['latest_market_date'])}｜營收期 {html.escape(meta['latest_revenue_period'])}｜財報季 {html.escape(meta['latest_financial_quarter'])}｜研究候選，不是買進建議</p></div>
<div class="cards">
